...
class BrokenMemoizingSupplier<T> implements Supplier<T> {
  ...
  public T get() {
    if (!initialized) {
      lock.lock();
      try {
        // This code is broken because initialized may have changed between the call to 'lock' and
        // now.
        ...
        (s) -> {
          ...
          Runnable test =
              () -> {
                ...
                if (supplier.get() != 1) {
                  throw new AssertionError("Memorizing supplier should only be initialized once");
                }
              };
